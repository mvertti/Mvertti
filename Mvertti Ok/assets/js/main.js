...
// 2. CLONACIÓN PARA BUCLE INFINITO (Triple Buffer)
// Clonamos el grupo completo para tener margen de maniobra en los extremos
...
// Lista actualizada con todos los elementos (clones + originales)
let todosLosSlides = Array.from(pista.querySelectorAll('.imagen-slide'));
let indiceActual = imagenesOriginales.length; // Empezamos en la primera imagen real

// 3. FUNCIÓN DE MOVIMIENTO Y ACTUALIZACIÓN
function actualizarGaleria(indice, conAnimacion = true) {
    if (indice < 0 || indice >= todosLosSlides.length) return;

    // Control de la transición (se apaga para los saltos invisibles del bucle)
    pista.style.transition = conAnimacion ? 'transform 0.7s cubic-bezier(0.25, 1, 0.5, 1)' : 'none';
    
    // Manejo de la clase active para opacidad
...
// 4. LÓGICA DE CLIC (Navegación + Lupa)
pista.addEventListener('click', (e) => {
    const slideClickeado = e.target.closest('.imagen-slide');
    if (!slideClickeado) return;
    
    const nuevoIndice = todosLosSlides.indexOf(slideClickeado);

    // Si la imagen ya tiene la clase 'active', abrimos la lupa
    if (slideClickeado.classList.contains('active')) {
        console.log("Abriendo lupa..."); // Esto te ayudará a ver si el clic funciona en la consola
        const img = slideClickeado.querySelector('img');
        if(img) {
            abrirLupa(img.src);
        }
    } else {
        // Si no está activa, movemos la galería
        actualizarGaleria(nuevoIndice);
    }
});

// 5. FUNCIONES DE LA LUPA (MODAL)
...
if(modal) modal.onclick = (e) => { if(e.target === modal) cerrarLupa(); };
document.addEventListener('keydown', (e) => { if (e.key === "Escape") cerrarLupa(); });

// 6. DETECCIÓN DE BUCLE (Salto invisible al terminar la transición)
pista.addEventListener('transitionend', () => {
    // Si llegamos a los clones del final, saltamos al inicio real
    ...
    // Si llegamos a los clones del principio, saltamos al final real
    else if (indiceActual < imagenesOriginales.length) {
        actualizarGaleria(indiceActual + imagenesOriginales.length, false);
    }
});

// 7. SOPORTE PARA SWIPE EN MÓVILES
let xInicial = null;
pista.addEventListener('touchstart', (e) => { 
    xInicial = e.touches.clientX; 
}, { passive: true });

pista.addEventListener('touchmove', (e) => {
    if (!xInicial) return;
    let diferenciaX = xInicial - e.touches.clientX;
    
    if (Math.abs(diferenciaX) > 50) { // Sensibilidad del swipe
        actualizarGaleria(diferenciaX > 0 ? indiceActual + 1 : indiceActual - 1);
        xInicial = null;
    }
}, { passive: true });

// 8. AJUSTES INICIALES Y RESIZE
window.addEventListener('load', () => {
    // Pequeño delay para asegurar que el navegador calculó bien los anchos
