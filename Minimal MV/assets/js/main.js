...
$(window).on('load', function() {
    ...
    // 4. MOTOR DE ZOOM INTELIGENTE (Clic, Pellizco y Arrastre Libre en Móvil)
    function asignarZoom() {
        pista.querySelectorAll('img').forEach(img => {
            if (!img.dataset.zoomActivo) {
                ...
                // --- 2. SENSORES DE PELLIZCO Y ARRASTRE (MÓVIL) ---
                ...
                img.addEventListener('touchstart', function(e) {
                    if (e.touches.length === 2) {
                        ...
                        // Iniciar arrastre con 1 dedo si la foto ya tiene zoom
                        isDragging = true;
                        startX = e.touches[0].clientX;
                        startY = e.touches[0].clientY;
                        
                        // Leer exactamente en qué parte de la foto estamos
                ...
                img.addEventListener('touchmove', function(e) {
                    // A. SI ESTÁN USANDO DOS DEDOS (PELLIZCO)
                    if (e.touches.length === 2 && distanciaInicialPellizco !== null) {
                        e.preventDefault(); // Bloqueamos comportamientos raros
                        ...
                        // Pellizco Normal (Alejar)
                        if (distanciaActual < distanciaInicialPellizco - 30) {
                            this.classList.remove('zoomed');
                        }
                    } 
                    // B. SI ESTÁN USANDO UN DEDO PARA MOVERSE POR LA FOTO ZOOMED
                    else if (e.touches.length === 1 && isDragging && this.classList.contains('zoomed')) {
                        e.preventDefault(); // Crucial: Apaga el scroll torpe del navegador

                        // Calculamos hacia dónde deslizó el dedo el usuario
                        const dx = e.touches[0].clientX - startX;
                        const dy = e.touches[0].clientY - startY;

                        // Mapeamos ese movimiento a la foto (1.5 es la velocidad de arrastre)
                        const sensibilidad = 1.5;
                        const moveX = (dx / this.offsetWidth) * 100 * sensibilidad;
                        const moveY = (dy / this.offsetHeight) * 100 * sensibilidad;

                        currentOriginX -= moveX;
                        currentOriginY -= moveY;

                        // Ponemos límites (0 a 100) para que no arrastre la foto fuera de la pantalla
                        ...
                        // Reseteamos las coordenadas para el siguiente milisegundo
                        startX = e.touches[0].clientX;
                        startY = e.touches[0].clientY;
                    }
                });
