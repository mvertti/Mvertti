...
$(window).on('load', function() {
    ...
    // 4. MOTOR DE ZOOM INTELIGENTE (Clic 1x en PC y Sensor de Pellizco en Móvil)
    function asignarZoom() {
        pista.querySelectorAll('img').forEach(img => {
            if (!img.dataset.zoomActivo) {
                ...
                // Un clic en computadora o un toque con 1 dedo en celular
                img.addEventListener('click', function(e) {
                    e.stopPropagation();
                    if (!this.classList.contains('zoomed')) {
                        // Quitamos el zoom a cualquier otra foto antes de acercar esta
                        pista.querySelectorAll('img').forEach(item => item.classList.remove('zoomed'));
                        
                        // Calculamos la posición del ratón para hacer zoom justo ahí
                        ...
                        // Si ya tiene zoom, con otro clic se lo quitamos
                        this.classList.remove('zoomed');
                    }
                });

                // --- 2. SENSOR DE PELLIZCO MULTITÁCTIL (MÓVIL) ---
                let distanciaInicialPellizco = null;

                img.addEventListener('touchstart', function(e) {
                    // Si el usuario toca la pantalla con exactamente 2 dedos
                    if (e.touches.length === 2) {
                        // Calculamos la distancia inicial entre los dos dedos
                ...
                img.addEventListener('touchmove', function(e) {
                    if (e.touches.length === 2 && distanciaInicialPellizco !== null) {
                        e.preventDefault(); // Bloqueamos el scroll de la página para que no interfiera
                        ...
                        // A. PELLIZCO INVERSO (Separar los dedos para Acercar)
                        if (distanciaActual > distanciaInicialPellizco + 30) {
                            if (!this.classList.contains('zoomed')) {
                                pista.querySelectorAll('img').forEach(item => item.classList.remove('zoomed'));
                                
                                // Calculamos el centro exacto entre los dos dedos
                                const rect = this.getBoundingClientRect();
                                ...
                                const xPercent = (centroX / rect.width) * 100;
                                const yPercent = (centroY / rect.height) * 100;
                        ...
                        // B. PELLIZCO NORMAL (Juntar los dedos para Alejar)
                        if (distanciaActual < distanciaInicialPellizco - 30) {
                            this.classList.remove('zoomed');
                        }
                    }
                ...
                img.addEventListener('touchend', function(e) {
                    // Si levantan uno o ambos dedos, reseteamos el sensor
                    if (e.touches.length < 2) {
